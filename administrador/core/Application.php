...
class Application
{
	...
	function run(){
		...
		//si la aplicacion requiere autenticacion y no este logeado, redirecciono al login. Deja pasar todas las acciones del login_controller
		if( $this->isEnableAuth() && !$this->userAuth() ){
			...
			if( isset($configs['login_controller']) ){
				...
				//si es una accion del controlador que se especifico como login la dejo pasar
				if($this->url_params['c'] == $configs['login_controller']['controller'] ){
					echo $controller->runAction($this->url_params['a'],null);
					exit();
				}else{					
				//lo mando al default del login
		...
		//si no se paso el controlador o la accion uso la default
		if( !isset($this->url_params['c']) || !isset($this->url_params['a']) ){
			$configs = include($_SERVER['PATH_BASE'] . DIRECTORY_SEPARATOR . DIRECTORY_CONF . DIRECTORY_SEPARATOR . 'config.php');
