...
<table>
	...
	<?php foreach ($listado as $id_fila => $fila){
		...
		echo '	<td>'.Link::html2("./?c=test&a=modificacion&id={$fila['id']}",'editar',null,$imagen).'</td>';

		$imagen = array('nombre'=>'botones/borrar.png', 'attr' => array('alt'=>'borrar','style'=>'width:30px;'));
		echo '	<td>'.Link::html2("./?c=test&a=borrar&id={$fila['id']}",'borrar',null,$imagen).'</td>';

		echo "</tr>";
	} ?>
</table>
<article id="botonera">
            <a href="./?c=test&a=alta"><p>NUEVO<img src="./img/botones/agregar.png"></p></a>
        </article>
<style type="text/css">
	table{
